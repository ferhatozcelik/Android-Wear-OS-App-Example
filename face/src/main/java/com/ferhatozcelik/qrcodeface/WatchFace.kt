...
class WatchFace :CanvasWatchFaceService() {
    ...
    private class EngineHandler(reference: WatchFace.Engine) :Handler(Looper.myLooper()!!) {
    ...
    inner class Engine :CanvasWatchFaceService.Engine() {
        ...
        override fun onCreate(holder: SurfaceHolder) {
            ...
            setWatchFaceStyle(WatchFaceStyle.Builder(this@WatchFace).setAcceptsTapEvents(true).build())
        ...
        override fun onTapCommand(tapType: Int, x: Int, y: Int, eventTime: Long) {
            when (tapType) {
                WatchFaceService.TAP_TYPE_TOUCH -> { // The user has started touching the screen.
                }
                WatchFaceService.TAP_TYPE_TOUCH_CANCEL -> { // The user has started a different gesture or otherwise cancelled the tap.
                }
                WatchFaceService.TAP_TYPE_TAP -> // The user has completed the tap gesture.
        ...
        override fun onDraw(canvas: Canvas, bounds: Rect) { // Draw the background.
            if (mAmbient) {
            ...
            val text =
                if (mAmbient) String.format("%d:%02d", mCalendar.get(Calendar.HOUR), mCalendar.get(Calendar.MINUTE))
                else String.format("%d:%02d:%02d", mCalendar.get(Calendar.HOUR), mCalendar.get(Calendar.MINUTE), mCalendar.get(Calendar.SECOND))
        ...
        override fun onApplyWindowInsets(insets: WindowInsets) {
            ...
            mXOffset = resources.getDimension(if (isRound) R.dimen.digital_x_offset_round
            else R.dimen.digital_x_offset)

            val textSize = resources.getDimension(if (isRound) R.dimen.digital_text_size_round
            else R.dimen.digital_text_size)
